<template>
  <div class="p-6 flex flex-col md:flex-row items-center md:justify-between relative">
    <div class="mb-4 md:mb-0 md:mr-4 text-white md:w-1/2 lg:w-2/3 xl:w-3/4">
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <img class="w-24 mr-2 mt-2" :src="currencyImg" :alt="`${cryptocurrencyName} logo`"> <!-- Adjusted margin-top for the logo -->
          <div>
            <p class="text-xl md:text-2xl lg:text-3xl font-bold text-center md:text-left">{{ cryptocurrencyName }}</p>
    ...
    <div class="text-white md:w-1/2 lg:w-1/3 xl:w-1/4">
        ...
        <a :href="'/dashboard/' + cryptocurrencyName.toLowerCase() + '/buy'" class="bg-purple-500 text-white px-4 py-2 rounded-md block text-center transition duration-300 ease-in-out transform hover:scale-105 active:scale-95 focus:outline-none">Buy Now</a>
    </div>
  </div>
</template>
